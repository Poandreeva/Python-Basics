...
pay, time, one, prize = argv
print('заработная плата сотрудника: ', float(time) * float(one) + float(prize))
...
old = input('введите исходный список: ').split()
new = [float(old[i]) for i in range(1, len(old)) if float(old[i-1]) < float(old[i])]   #float, чтобы можно было нецелые числа использовать
print('новый список, в котором элементы исходного списка, значения которых больше предыдущего элемента: ', new)
...
old = input('введите список чисел: ').split()
new = [el for el in old if old.count(el) == 1]
print(new)
...
li = [i for i in range(100, 1001) if i % 2 == 0]  # реализация списка
print(li)
...
n = int(input('введите число для получeния факториалов всех числе от 1 до введенного: '))
for el in fact(n):
    print(el)
